  ...
  <div class="grid">
    <BaseCard>
      ...
      <form class="row" @submit.prevent="onSave">
        ...
        <div><label>Día</label><select v-model="form.dia"><option value=''>Seleccionar…</option><option>Lunes</option><option>Martes</option><option>Miércoles</option><option>Jueves</option><option>Viernes</option><option>Sábado</option></select></div>
        <div><label>Hora</label><input v-model="form.hora" type="text" placeholder="18:00"/></div>
        <div><label>Cupo</label><input v-model="form.cupo" type="number" placeholder="20"/></div>
      </form>
      <template #footer>
    ...
    <BaseCard>
      ...
      <table class="table">
        <thead><tr><th>Nombre</th><th>Día</th><th>Hora</th><th>Cupo</th></tr></thead>
        <tbody>
